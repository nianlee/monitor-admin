...
const findParentId = (datas, id) => {
  for (let data of datas) {
    if (data.childrenList && data.childrenList.findIndex(item => item.id === id) > -1) {
      return data.id
    }
  }
  return null
...
class MenuSelect extends Component {
  ...
  level1Change(e) {
    const ids = findChildIds(this.props.dataSource, e.target.value).concat(e.target.value)
    if (e.target.checked) {
  ...
  level2Change(e) {
    ...
    if (e.target.checked) {
      ...
      const hasChildIn = ids.findIndex(item => siblings.indexOf(item) > -1) > -1
  ...
  render() {
    return (<div className={styles.checkboxWapper}>
      {this.renderCheckBox()}
    </div>)
  }
}
