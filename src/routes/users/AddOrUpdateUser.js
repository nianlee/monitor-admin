...
const AddOrUpdateUser = ({ form, dispatch, addOrUpdateUser }) => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        if (addOrUpdateUser.type == "add") {
          ...
          dispatch({
            type: "addOrUpdateUser/modifyUserInfo",
            payload: {
              ...payload,
  ...
  const areaLoadData = selectedOptions => {
    dispatch({
      type: "addOrUpdateUser/queryAreaByParentCode",
          ...
          <Col span={8}>
            <FormItem {...formItemLayout} label="角色">
              {getFieldDecorator("roleId", {
                ...
                >
                  {addOrUpdateUser.roleList &&
                    addOrUpdateUser.roleList.map(role => (
...
export default connect(({ addOrUpdateUser }) => ({ addOrUpdateUser }))(
